...
const NFC=100;
const bundlePrice=(q:number)=>q===2?699:q>=3?949:399;
export default function ProductOptions({slug,price}:{slug:string;price:number}){
 ...
 return <div className="purchase-options">
  ...
  <div className="quantity-options">{[1,2,3].map(x=><button type="button" key={x} className={q===x?'quantity-choice active':'quantity-choice'} onClick={()=>setQ(x)}>{x===2&&<em className="popular-badge">MOST POPULAR</em>}<span>{x} {x===1?'Wallet':'Wallets'}</span><b>{bundlePrice(x).toLocaleString()} EGP</b>{x>1&&<small>Save {((399*x)-bundlePrice(x)).toLocaleString()} EGP</small>}</button>)}</div>
  ...
  <Link className="mobile-sticky-buy" href={checkoutHref} aria-label={`Buy ${q} ${q===1?'wallet':'wallets'} for ${total.toLocaleString()} EGP`}><span>{q} {q===1?'Wallet':'Wallets'}</span><strong>{total.toLocaleString()} EGP</strong><b>BUY NOW</b></Link>
 </div>
}
