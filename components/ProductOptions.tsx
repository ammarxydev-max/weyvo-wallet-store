...
const NFC=100;
const bundlePrice=(q:number)=>q===2?899:q>=3?1199:499;
export default function ProductOptions({slug,price}:{slug:string;price:number}){
 ...
 return <div className="purchase-options">
  ...
  <div className="quantity-options">{[1,2,3].map(x=><button type="button" key={x} className={q===x?'quantity-choice active':'quantity-choice'} onClick={()=>setQ(x)}><span>{x} {x===1?'Wallet':'Wallets'}</span><b>{bundlePrice(x).toLocaleString()} EGP</b>{x>1&&<small>Save {((499*x)-bundlePrice(x)).toLocaleString()} EGP</small>}</button>)}</div>
